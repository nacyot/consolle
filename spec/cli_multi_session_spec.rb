...
RSpec.describe 'Multi-session support' do
  ...
  describe '#save_sessions' do
    it 'adds schema version and writes to file' do
      sessions = {
        'cone' => { 'process_pid' => 12_345 },
        'dev' => { 'process_pid' => 67_890 }
      }
      ...
      expect(saved_data['cone']['process_pid']).to eq(12_345)
      expect(saved_data['dev']['process_pid']).to eq(67_890)
    end
  end
  ...
  describe '#process_alive?' do
    ...
    it 'returns false for non-existent process' do
      expect(cli.send(:process_alive?, 999_999)).to be false
    end
