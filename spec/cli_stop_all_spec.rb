  ...
  describe "#stop_all" do
    ...
    context "when active sessions exist" do
      ...
      before do
        ...
        allow(cli).to receive(:process_alive?).with(12345).and_return(true)
        allow(cli).to receive(:process_alive?).with(12346).and_return(true)
        allow(cli).to receive(:process_alive?).with(12347).and_return(false)
      ...
      it "logs session stop events" do
        ...
        expect(cli).to receive(:log_session_event).with(12345, "session_stop", {
          reason: "stop_all_requested"
        })
        expect(cli).to receive(:log_session_event).with(12346, "session_stop", {
          reason: "stop_all_requested"
        })
